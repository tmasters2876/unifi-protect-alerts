...
PROMPT = (
    ...
    "For animals: name the species if identifiable, and count if more than one.\n"
    "For vehicles: type, color, and make/model only if confidently legible.\n"
    "Avoid 'left/right'; use 'toward/away from camera' or 'toward the door/yard' only if obvious.\n"
    "Compose `notification_message` as 1-2 natural sentences that read like a witness description, "
    "combining ALL subjects together, e.g. 'An Asian male and a raccoon are at the front door. "
    "The Asian male is wearing a blue shirt (polo style) and is carrying an AR-type weapon.'\n"
