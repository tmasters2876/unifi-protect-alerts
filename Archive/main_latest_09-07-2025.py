...
def _kind_label(types: Set[str], summary: Optional[str]) -> str:
    # 1) Trust UniFi smart detections first.
    ...
    # 2) Positive + not-negated checks per entity
...
async def _resolve_camera_name(payload: Dict[str, Any]) -> Optional[str]:
    # 1) If the webhook includes a friendly name, use it
    nm = _find_camera_name_in_payload(payload)
    if nm:
        return nm

    # 2) Prefer a 24-char integration camera id from the payload…
    cid = _find_camera_id(payload)
    if not (isinstance(cid, str) and len(cid) == 24):
        # …or extract it from any thumbnail/snapshot URL
        cid = _extract_camera_id_from_urls(payload)

    # 3) Map id → name (Integration API cache)
...
async def unifi_webhook(req: Request):
    ...
    if not camera_name:
        # Log urls and extracted id so we can tune if needed
        urls = _list_image_urls(payload)
    ...
    if summary and summary.lower().startswith("alert:"):
    ...
    title = title_base + (f" — {camera_name}" if camera_name else "")
